...
function rowFromParsed_(ts, p, raw) {
  ...
  var recargaSi = String(p.recargaUso || "").toLowerCase().indexOf("sí") >= 0 ||
    String(p.recargaUso || "").toLowerCase().indexOf("si") >= 0;
  ...
  return [
    ...
    show_(p.recargaUso),
    recargaSi ? show_(p.recargaExp) : NA,
    recargaSi ? show_(p.recargaMejora) : NA,
    show_(p.popUso),
